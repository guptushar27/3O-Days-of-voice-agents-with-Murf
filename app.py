...
from schemas.responses import (TTSResponse, TranscriptionResponse, LLMResponse,
                               ConversationResponse, ChatHistoryResponse,
                               AudioUploadResponse)
...
def test_websocket():
    """Serve the WebSocket testing interface"""
...
def ws_status():
    ...
    return jsonify({
        ...
        'active_sessions': ws_service.get_active_sessions_count(),
...
def generate_tts():
    ...
    try:
        ...
        # Generate audio
        result = tts_service.generate_speech(tts_request.text,
        ...
        if result['success']:
            ...
            logger.info(
                f"TTS generated successfully using {result.get('service_used', 'unknown')}"
            )
        else:
        ...
        if result['success']:
            ...
            response = TTSResponse(success=False,
                                   error=result['error'],
    ...
    try:
        if 'audio' not in request.files:
            return jsonify({
                ...
                False,
                'fallback_message':
                'Please provide an audio file for transcription'
            }), 400
    ...
    try:
        ...
        if 'audio' not in request.files:
            fallback_audio = tts_service.create_fallback_audio(
                "Audio input is required for this service.")
            return jsonify({
        ...
        if not transcription_result['success']:
            ...
            fallback_response = transcription_result.get(
                'fallback_message',
        ...
        llm_result = llm_service.generate_response(
            chat_sessions[session_id]['messages'], user_message)
        ...
        if not tts_result['success']:
            ...
            fallback_audio = tts_service.create_fallback_audio(
                llm_response[:100] +
                "... I'm having trouble with voice generation.")
...
def serve_audio_file(filename):
    ...
    try:
        audio_path = os.path.join(app.instance_path, 'static', 'audio',
                                  filename)
...
def handle_connect():
    ...
    ws_service.register_session('default', request.sid)

    emit(
        'status', {
            'message': 'Connected to VoxAura WebSocket server',
            'connected': True,
...
def handle_disconnect():
    ...
    ws_service.unregister_session(request.sid)


@socketio.on('message')
def handle_message(data):
    """Handle incoming WebSocket messages - Echo back the message"""
    logger.info(f"Received message from {request.sid}: {data}")

    response_data = ws_service.handle_echo_message(request.sid, data)
...
def handle_audio_stream(data):
    ...
    logger.info(f"Received audio stream from {request.sid}")

    response = ws_service.handle_audio_stream(request.sid, data)
...
def handle_chat_message(data):
    ...
    response = ws_service.handle_chat_message(request.sid, data)
    emit('chat_response', response)


@socketio.on('register_session')
def handle_register_session(data):
    """Register a chat session with WebSocket"""
    session_id = data.get('session_id', 'unknown')
    ws_service.register_session(session_id, request.sid)
